...
- **Universal Library Coding Standards vs Engine-Specific Module Loading**:
  - The coding standards, purity rules, reification principles (`library(reif)`: `=(X, Y, Truth)`, `cond_t`, `if_`), integer constraints (`CLP(Z)` / `CLP(FD)`), DCGs (`library(dcgs)`), character I/O (`chars`), safe type testing (`library(si)`), higher-order lambdas (`library(lambda)` / `library(yall)`), formatted output (`library(format)`), list utilities (`library(lists)`), association maps (`library(assoc)`), and range iteration (`library(between)`) apply **universally as general Prolog coding standards** across all supporting engines.
  - While module load headers (e.g. `:- use_module(library(clpz)).` in Scryer vs `:- use_module(library(clpfd)).` in SWI) are dialect-specific or idiosyncratic to a particular engine, each engine dialect skill MUST document that engine's load mechanism while explicitly cross-referencing the universal Prolog coding standards ([`prolog-conventions`](.agents/skills/prolog-conventions/SKILL.md), [`prolog_guidelines.md`](.agents/references/prolog_guidelines.md)) for code style, purity, and usage rules.
