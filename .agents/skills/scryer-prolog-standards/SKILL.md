...
| **Reified Logic** | `:- use_module(`[`library(reif)`](https://github.com/mthom/scryer-prolog/blob/master/src/lib/reif.pl)`).` | `if_/3`, `cond_t/3`, `dif/2`, `(=)/3`, `memberd_t/3`, `tfilter/3` | Pure reified conditional testing. Prefer direct reified predicates (`=/3`, `memberd_t/3`) for booleans and aggressively prefer `cond_t` over `if_` / `->` for DRY value selection. |
