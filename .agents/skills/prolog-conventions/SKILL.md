...
4. **Higher-Order Logic & Lambdas**: Prefer passing partial goals/closures to higher-order predicates (`call/N`, `maplist/N`, `foldl/N`, `tfilter/3`, `include/3`, `exclude/3`) and `library(lambda)` (`\X^...`, `\X^Y^Goal`) over writing primitive recursive list traversals:
   ```prolog
