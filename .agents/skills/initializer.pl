...
%%  init_project_spec(+ProjectName:chars, +System:atom, -Files:list) is det.
%   Generates abstract file specification list for a new Prolog project.
%   Files is a list of file(Path, ContentChars).
init_project_spec(ProjectName, System, Files) :-
    bakage_manifest(ProjectName, "0.1.0", BakageChars, []),
    pack_manifest(ProjectName, "0.1.0", PackChars, []),
    starter_module(ProjectName, System, ModChars, []),
    test_scaffold(ProjectName, System, TestChars, []),
    readme_scaffold(ProjectName, System, ReadmeChars, []),
    (   System = swi ->
...
%%  generate_module_spec(+ModuleName:chars, +System:atom, -ContentChars:chars) is det.
%   Generates content for a new Prolog module stub.
generate_module_spec(ModuleName, System, ContentChars) :-
    phrase(starter_module(ModuleName, System), ContentChars).

%%  generate_template_spec(+ProjectName:chars, +System:atom, -Files:list) is det.
%   Generates file specifications for standard project template layout.
generate_template_spec(ProjectName, System, Files) :-
    init_project_spec(ProjectName, System, Files).
...
starter_module(ModuleName, System) -->
    (   { System = swi } ->
        ":- module(", ModuleName, ", [\n",
        "    hello/1,\n",
        "    parse_item//1,\n",
...
test_scaffold(ProjectName, System) -->
    (   { System = swi } ->
...
readme_scaffold(ProjectName, System) -->
    "# ", ProjectName, "\n\n",
    "Prolog project `", ProjectName, "` initialized for Prolog system `", System, "`.\n\n",
    "## Quick Start\n\n",
    "### Running Unit Tests\n\n",
    (   { System = swi } ->
