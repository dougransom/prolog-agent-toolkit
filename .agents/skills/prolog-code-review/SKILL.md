...
| :--- | :--- |
| **Logical Purity & Reified Traversals** | - Are `library(reif)` predicates preferred over raw builtins wherever a reified equivalent exists? (e.g. `=(X, Y, Truth)` not `X = Y` as a test; `dif(X, Y)` not `\+ (X = Y)` or `X \= Y`; `memberd_t/3` not `member/2`; `tfilter/3` not `include/3`; `if_/3` not `-> ;`; `cond_t` not repeated `if_` assignments?)<br>- Is `:- use_module(library(reif)).` declared at the top of any file using these predicates?<br>- Are cuts (`!`), negation-as-failure (`\+/1`), and soft cuts (`->`) avoided in favor of `if_/3` from `library(reif)` or `dif/2` (preferring `dif(X, Y)` over `\+ (X = Y)`)? Are cuts (`!`), `\+/1`, `->` omitted for performance tuning? If introduced for *correctness*, is an explicit inline comment present justifying why pure logic constructs (`if_/3`, `dif/2`) were insufficient?<br>- Are direct reified predicates (e.g., `=(X, Y, Truth)`, `memberd_t/3`, `dif/3`) used directly when binding booleans instead of wrapping boolean assignments inside `if_/3`?<br>- Is `cond_t` aggressively preferred over `if_` and `->` when selecting between choices or values to adhere to the DRY principle (avoiding repeated variable unifications across true/false branches)?<br>- Are `tfilter/3` and `tpartition/4` used for pure reified list filtering/partitioning instead of non-reified `include/3`/`exclude/3` or manual recursive loops? |
