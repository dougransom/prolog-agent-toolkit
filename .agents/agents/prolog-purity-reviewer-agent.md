...
3. **Reification & Sound Inequality Audit**: Verify conditionals use `if_/3` from `library(reif)`, `dif/2` (preferring `dif(X, Y)` over negation-as-failure `\+ (X = Y)`), and `zcompare/3` (for reified arithmetic comparison) instead of standard `->` conditionals, impure `\+/1`, or unindexed cuts.
