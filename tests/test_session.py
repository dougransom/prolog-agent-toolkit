...
@pytest.fixture
def trealla_engine():
    engine_bin = shutil.which("tpl")
    if not engine_bin:
        pytest.skip("tpl binary not found on PATH")
    return "trealla"
...
def test_session_basic_query_trealla(trealla_engine):
    with PrologSession(engine=trealla_engine, timeout="3s") as session:
        assert session.is_alive()
        res = session.query("X = 42.")
        assert isinstance(res, QueryResult)
        assert res.timed_out is False
...
def test_session_successive_queries_trealla(trealla_engine):
    with PrologSession(engine=trealla_engine, timeout="3s") as session:
        assert session.is_alive()
...
def test_session_consult_file_trealla(trealla_engine):
    with tempfile.NamedTemporaryFile("w", suffix=".pl", delete=False) as f:
        f.write("test_fact(999).\n")
        f_path = f.name

    try:
        with PrologSession(engine=trealla_engine, files=[f_path], timeout="3s") as session:
            res = session.query("test_fact(X).")
            assert res.timed_out is False
